...
function getCreatedScrapId(response: unknown): number | null {
  ...
  return data.scrapId ?? data.id ?? data.data?.scrapId ?? data.data?.id ?? null;
}
...
export default function CourseDetailPage() {
  ...
  const ensureCourseSaved = async () => {
    ...
    if (!result) throw new Error('코스 결과가 없습니다.');
    ...
    setCurrent({ courseId: saved.id });
    return saved.id;
  };

  const { requestSave, saveToScrapElement } = useSaveToScrap(
    async (groupId) => {
      if (!result || scrapBusy) return;
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('[CourseDetailPage] share failed:', e);
      alert('공유에 실패했습니다.');
    } finally {
      setShareBusy(false);
    }
      ...
      <div className="flex w-full items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex size-6 items-center justify-center transition-colors disabled:opacity-50 ${
